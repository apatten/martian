...
describe('Learning Path API', () => {
    ...
    describe('Manager tests', () => {
        ...
        describe('create learning path failures', () => {
            ...
            it('invalid name', () => {
                return lpm
                    .createLearningPath({ name: '' })
                    .catch(failed)
                    .then(() => expect(failed).toHaveBeenCalled());
            });
            it('invalid title', () => {
                return lpm
                    .createLearningPath({ name: 'foo', title: '' })
                    .catch(failed)
                    .then(() => expect(failed).toHaveBeenCalled());
            });
            it('invalid summary', () => {
                return lpm
                    .createLearningPath({ name: 'foo', title: 'bar', summary: 123 })
                    .catch(failed)
                    .then(() => expect(failed).toHaveBeenCalled());
            });
            it('invalid category', () => {
                return lpm
                    .createLearningPath({ name: 'foo', title: 'bar', category: 123 })
                    .catch(failed)
                    .then(() => expect(failed).toHaveBeenCalled());
            });
        });
    ...
    describe('operations', () => {
        ...
        it('can fail updating a learning path (invalid category)', () => {
            const failed = jest.fn();
            return lp
                .update({ title: 'path', category: 132 })
                .catch(failed)
                .then(() => expect(failed).toHaveBeenCalled());
        });

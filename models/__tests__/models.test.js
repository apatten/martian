    ...
    describe('Property checking', () => {
        it('has valid properties', () => {
            function testModelItem(modelItem, validProps) {
                const invalidProp = Object.keys(modelItem).find(
                    prop => !validProps.includes(prop) || typeof modelItem[prop] === 'undefined'
                );
                if (invalidProp) {
            ...
            allModels.forEach(model => {
                ...
                model.forEach(modelItem =>
                    testModelItem(modelItem, ['field', 'name', 'isArray', 'transform', 'constructTransform'])
                );
            });
        });
                ...
                models.forEach(model => {
                    const modelMap = (function modelToObj(currentModel) {
                        ...
                        return modelArr.reduce((modelMapping, fieldObj) => {
                            ...
                            if (
                                fieldObj.transform &&
                                typeof fieldObj.transform === 'object' &&
                                !allModels.includes(fieldObj.transform)
                            ) {

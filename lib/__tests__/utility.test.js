...
describe('Martian utility', () => {
    ...
    it('can get an appropriately-encoded user token', () => {
        expect(utility.getUserToken('user 1')).toBe('=user%25201');
        expect(utility.getUserToken(123)).toBe(123);
        expect(utility.getUserToken('user:token')).toBe('user:token');
        expect(() => utility.getUserToken()).toThrow();
    });
    ...
    it('can base64 encode and decode a string (browser)', () => {
        const rawString = 'abcd 1234';
        const b64String = utility.base64.encode(rawString);
        expect(utility.base64.decode(b64String)).toBe(rawString);
    });
});
